...
function clickAbleStop(){
  for (var j = 0; j < markerArray.length; j++) {
    markerArray[j].content = ""; 
    markerArray[j].index = j; 
    var marker = markerArray[j];
    marker.addListener('click', function() {
      linkOfRequest = 'https://chicken-of-the-sea.herokuapp.com/redline/schedule.json?stop_id='+ tStops[this.index][3];
      ...
      request.onreadystatechange = function() {
        if (request.readyState == 4 && request.status == 200) {
          ...
          var pos;
          if (dataArray == null){
            alert("Too many requests! Please wait a second and try again...");
          }
          for (var i = 0; i<dataArray.length-1;i++) {
            var  dir = dataArray[i].attributes.direction_id;
            var arrive =  dataArray[i].attributes.arrival_time;
            var depart =  dataArray[i].attributes.departure_time;  
            var arriveString;
          ...
          scheduleStr = "<u>Time</u> &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp <u>Direction</u></br>" + scheduleStr;

          var displayStr = "This stop is: <b>" + tStops[markerArray[this.index].index][0] + "</b>. </br> Here is the upcoming schedule: <br> <br>" + scheduleStr;
          infowindow.setPosition(markerArray[this.index].getPosition());
...
function findClosestStop(){
  ...
  me = new google.maps.Marker({
    position: me,
    title: "This is your current location.</br> The closest T Stop to you is: " + tStops[minDistanceStopIndex][0] + ".</br> It is: " + minDistance + " miles away from you right now.",
    map: map
  });
